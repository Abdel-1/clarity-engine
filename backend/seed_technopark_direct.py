...
def seed_technopark():
    ...
    client = db.query(Client).filter(Client.company_name == "Technopark").first()
    if not client:
        client = Client(company_name="Technopark", sector="Technology")
        db.add(client)
        db.commit()
        db.refresh(client)
    ...
    existing = db.query(BrandSystem).filter(BrandSystem.brand_name == "Technopark").first()
    if not existing:
        bs = BrandSystem(**data)
        db.add(bs)
        db.commit()
        print("✅ Technopark brand system seeded.")
    else:
        print("ℹ️ Technopark brand system already exists.")
    db.close()
