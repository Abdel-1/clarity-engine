...
def login(email: str, password: str, db: Session = Depends(get_db)):
    ...
    token = create_access_token({"user_id": user.id, "role": getattr(user, "role", "client"), "email": user.email})
    return {"access_token": token, "token_type": "bearer", "role": getattr(user, "role", "client")}
